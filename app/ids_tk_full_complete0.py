#!/usr/bin/env python3
"""
ids_tk_full_complete.py  (Final patched GUI)
- Full 41 inputs (correct order)
- Presets (auto-filled from train_clean.csv group means)
- Simulation, logging, charts, export
- Uses models/random_forest_fixed_41.pkl and models/scaler_fixed_41.pkl
"""
...
if not os.path.exists(MODEL_PATH) or not os.path.exists(SCALER_PATH):
    msg = f"Missing model or scaler. Expected:\n{MODEL_PATH}\n{SCALER_PATH}"
    print(msg)
...
# --------- Load train CSV and compute presets by attack group ----------
presets_means = {}
global_means = None
...
if os.path.exists(TRAIN_CSV):
    try:
        ...
        if missing:
            print("Warning - training CSV missing columns:", missing)
        # compute global means (fallback)
        global_means = df_train[FEATURES].mean().to_dict()

        # If attack_type exists, try building group means for categories
        if "attack_type" in df_train.columns:
            # Normalize attack_type strings to lower
            df_train["attack_type_lc"] = df_train["attack_type"].astype(str).str.lower()
            # Normal
            if (df_train["attack_type_lc"] == "normal").any():
                presets_means["Normal"] = df_train[df_train["attack_type_lc"] == "normal"][FEATURES].mean().to_dict()
            # DoS: look for many known DoS names
            dos_mask = df_train["attack_type_lc"].str.contains("dos|smurf|neptune|back|teardrop|land|pod", na=False)
            if dos_mask.any():
                presets_means["DoS"] = df_train[dos_mask][FEATURES].mean().to_dict()
            # Probe
            probe_mask = df_train["attack_type_lc"].str.contains("probe|ipsweep|satan|nmap|portsweep|mscan|saint", na=False)
            if probe_mask.any():
                presets_means["Probe"] = df_train[probe_mask][FEATURES].mean().to_dict()
            # R2L
            r2l_mask = df_train["attack_type_lc"].str.contains("r2l|ftp_write|guess_passwd|imap|phf|spy|warezclient|warezmaster", na=False)
            if r2l_mask.any():
                presets_means["R2L"] = df_train[r2l_mask][FEATURES].mean().to_dict()
            # U2R
            u2r_mask = df_train["attack_type_lc"].str.contains("u2r|buffer_overflow|loadmodule|perl|rootkit|sqlattack", na=False)
            if u2r_mask.any():
                presets_means["U2R"] = df_train[u2r_mask][FEATURES].mean().to_dict()
        else:
            # if no attack_type, try grouping by label (if label 21 common as normal)
            if "label" in df_train.columns:
                labels = df_train["label"].unique().tolist()
                # try to pick largest class as normal
                counts = df_train["label"].value_counts()
                top_label = counts.idxmax()
                presets_means["Normal"] = df_train[df_train["label"] == top_label][FEATURES].mean().to_dict()
            # fallback: assign global mean to all presets
    ...
    # no train csv: use zeros
    global_means = {f: 0.0 for f in FEATURES}

# If any preset missing, fill from global means
for preset_name in ["Normal", "DoS", "Probe", "R2L", "U2R"]:
    if preset_name not in presets_means:
        presets_means[preset_name] = global_means.copy()
...
preset_combo = ttk.Combobox(form_frame, textvariable=preset_var, values=list(presets_means.keys()), state="readonly", width=20)
...
# --------- Helpers: logging and reading logs ----------
def ensure_log_dir():
...
def collect_input_row():
    ...
    for f in FEATURES:
        ...
        try:
            ...
            # if user put text, try to replace commas etc
            t = txt.replace(',', '.').strip()
            try:
                row[f] = float(t)
            except:
...
def do_predict(row_df):
    # row_df already numeric and in correct column order
...
def update_ui_after_prediction(pred, conf):
    # display in listbox and status
    human = None
    # try to map normal vs attack by looking in train CSV if available
    if os.path.exists(TRAIN_CSV):
        try:
            df = pd.read_csv(TRAIN_CSV)
            if "label" in df.columns and "attack_type" in df.columns:
                mapping = df.groupby("label")["attack_type"].agg(lambda s: s.mode().iat[0] if not s.mode().empty else s.iloc[0])
                human = mapping.to_dict().get(pred, None)
        except Exception:
            human = None
    text = f"{pred}" + (f" ({human})" if human else "")
    status_var.set(f"Last: {text} — {conf}%")
    ts = datetime.now().strftime("%H:%M:%S")
    logs_listbox.insert(0, f"{ts} → {text} [{conf}%]")
...
def predict_action(event=None):
    try:
        ...
        refresh_charts()
        # popup
        if str(pred).lower() == "21" or (isinstance(pred, (int, np.integer)) and pred == 21):
...
def refresh_charts():
    logs = read_logs()
    ax1.clear(); ax1.set_facecolor('#0b1220')
    if logs.empty:
    ...
    ax2.clear(); ax2.set_facecolor('#0b1220')
    if logs.empty:
        ax2.text(0.5,0.5,"No logs yet", color='white', ha='center')
    else:
        last = logs.tail(80)
        try:
            if "timestamp" in last.columns:
...
def simulate_loop(interval=1.0):
    while simulate_flag.is_set():
        ...
        # fill with that mean slightly jittered
...
def export_csv():
    ...
    if df.empty:
        ...
        return
    fname = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV","*.csv")], initialfile="ids_export.csv")
    if not fname:
        return
